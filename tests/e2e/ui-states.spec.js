...
test.describe("SearchHistory", () => {
  ...
  test("after a search, scenario appears in history and clicking Re-run re-populates the input", async ({
    page,
  }) => {
...
test.describe("CriminalCodeExplorer", () => {
  ...
  test("section with enriched data can be expanded to show Definition", async ({
    ...
    await enrichedTag
      .locator("xpath=ancestor::div[contains(@style,'padding: 14px')]")
      .first()
      .click();

    // Definition label should appear in expanded view
    await expect(
      page.getByText("Definition", { exact: true }).first(),
...
test.describe("ErrorMessage", () => {
  ...
  test("shows error message with non-empty text when analyze returns 500", async ({
    page,
  }) => {
    await page.route("/api/analyze", async (route) => {
      await route.fulfill({
    ...
    await expect(
      page.getByText("Internal server error. Please try again."),
    ).toBeVisible();
...
test.describe("StagedLoading", () => {
  ...
  test("loading indicator is visible while analyze request is in flight", async ({
    page,
  }) => {
    await page.route("/api/analyze", async (route) => {
