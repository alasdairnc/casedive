...
test.describe("Live: UI Smoke", () => {
  test("page loads and shows core UI elements", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="research-submit"]'),
    ).toBeDisabled();
  });
...
test.describe("Live: Real API — Break and Enter", () => {
  test("returns results for a residential break and enter scenario", async ({
    page,
  }) => {
    await page.goto("/");
    await page
...
test.describe("Live: Real API — Assault", () => {
  test("returns results for an assault causing bodily harm scenario", async ({
    page,
  }) => {
    await page.goto("/");
    await page
...
test.describe("Live: Criminal Code Explorer", () => {
  test("opens explorer and finds results for 'assault'", async ({ page }) => {
    ...
    await expect(page.locator("body")).toContainText("assault", {
      timeout: 5000,
    });
  });
});
