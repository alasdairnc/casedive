...
export function WindowPanel({ label, windowStats, thresholds, t }) {
  ...
  const noVBadge = severityForMetric(
    rates.noVerifiedRate,
    thresholds?.noVerifiedRate1h,
  );
  const fallbackBadge = severityForMetric(
    rates.fallbackPathRate,
    thresholds?.fallbackPathRate1h,
  );
  ...
  return (
    ...
    >
      ...
      >
        <div>
          Samples (operational / quality / latency): {num(samples.operational)}{" "}
          / {num(samples.quality)} / {num(samples.latency)}
        </div>
      ...
      <BarRow
        label={`Error rate (${pct(err)})`}
        value={err}
        max={barMax}
        t={t}
        color={errBadge.color}
      />
      <BarRow
        label={`No-verified rate (${pct(noV)})`}
        value={noV}
        max={barMax}
        t={t}
        color={noVBadge.color}
      />
      <BarRow
      ...
      {rates.candidateSourceMix && (
        ...
        >
          Source mix (AI/Landmark/Local): {pct(rates.candidateSourceMix.ai)} /{" "}
          {pct(rates.candidateSourceMix.landmark)} /{" "}
          {pct(rates.candidateSourceMix.localFallback)}
        </div>
      )}
      <div
        style={{
      ...
      {Array.isArray(windowStats?.breakdowns?.byIssue) &&
        windowStats.breakdowns.byIssue.length > 0 && (
          <div style={{ marginTop: 12 }}>
            ...
            {windowStats.breakdowns.byIssue.map((row) => (
              ...
              >
                {row.issuePrimary}: req {num(row.requests)} | fallback{" "}
                {pct(row.fallbackPathRate)} | no-verified{" "}
                {pct(row.noVerifiedRate)}
              </div>
            ))}
          </div>
        )}
